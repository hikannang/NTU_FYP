  ...
  if (errorMessage) {
    ...
    if (errorText) {
      errorText.textContent =
        message ||
        "Sorry, we couldn't load this booking's details. Please try again later.";
    }
  }
}
  ...
  try {
    ...
    try {
      ...
      if (bookingDoc.exists()) {
        ...
        console.log(
          "Booking not found in user's collection, trying main bookings collection"
        );
      }
    } catch (err) {
...
function updateBookingUI() {
  try {
    ...
    const startTime =
      bookingData.start_time instanceof Date
    ...
    const endTime =
      bookingData.end_time instanceof Date
    ...
    if (bookingIdElement && bookingData.id) {
      // Remove "booking_" prefix if it exists
      const formattedId = bookingData.id.startsWith("booking_")
        ? bookingData.id.substring(8) // Remove first 8 characters ("booking_")
        : bookingData.id;
    ...
    if (carSeatsElement) {
      carSeatsElement.textContent = `${
        carData.seating_capacity || "4-5"
      } seats`;
    }
    ...
    if (durationElement) {
      ...
      if (minutes > 0) {
        durationText += `${hours > 0 ? " " : ""}${minutes} minute${
          minutes !== 1 ? "s" : ""
        }`;
      }
...
function addToCalendar() {
  try {
    ...
    // Get car details
    const carName =
      carData.model_data?.name ||
      (carData.car_type ? carData.car_type.split("_")[0] : "Car");
    const location = carData.address || bookingData.pickup_location || "";

    // Create calendar URL
    const calendarUrl = `https://www.google.com/calendar/render?action=TEMPLATE&text=CarShare: ${carName} Booking&dates=${start}/${end}&details=Booking reference: ${bookingId.slice(
      -6
