...
import { doc, getDoc, getDocs, collection, query, where, Timestamp } from "https://www.gstatic.com/firebasejs/9.17.1/firebase-firestore.js";
...
function displayAdditionalFeatures() {
    ...
    if (!featuresContainer || !carData.features || !Array.isArray(carData.features) || carData.features.length === 0) {
        return;
    }
...
function importDashboardSearchParams() {
    try {
        ...
        if (durationHoursSelect && typeof searchParams.durationHours !== 'undefined') {
            durationHoursSelect.value = searchParams.durationHours;
        }
        
        if (durationMinutesSelect && typeof searchParams.durationMinutes !== 'undefined') {
...
async function updateBookingSummary() {
    ...
    const durationHours = durationHoursSelect ? parseInt(durationHoursSelect.value) : 1;
    const durationMinutes = durationMinutesSelect ? parseInt(durationMinutesSelect.value) : 0;
    
    // Create date objects
    const startDate = new Date(`${date}T${hours.toString().padStart(2, "0")}:${minutes.toString().padStart(2, "0")}:00`);
    
    // Calculate total duration in minutes
    const totalDurationMinutes = days * 24 * 60 + durationHours * 60 + durationMinutes;
    ...
    if (summaryTotalElement) summaryTotalElement.textContent = totalPrice.toFixed(2);
}
...
async function checkAvailability() {
    try {
        ...
        if (startDateTime < now) {
            return { available: false, error: "Please select a pickup time in the future" };
        }
        ...
        bookingsSnapshot.forEach(doc => {
            ...
            if (data.status !== 'cancelled') {
                bookings.push({
                    id: doc.id,
                    ...data,
                    start: data.start_time.toDate ? data.start_time.toDate() : new Date(data.start_time.seconds * 1000),
                    end: data.end_time.toDate ? data.end_time.toDate() : new Date(data.end_time.seconds * 1000)
                });
            }
        });
        ...
        for (const booking of bookings) {
            ...
            const bookingStartMinus15 = new Date(booking.start.getTime() - 15 * 60000);
...
async function updateAvailabilityStatus() {
    ...
    try {
        ...
        if (availabilityResult.available) {
            ...
            let message = availabilityResult.error || "Not available for selected time";
            ...
            if (availabilityResult.conflictStart && availabilityResult.conflictEnd) {
                ...
                const conflictStart = availabilityResult.conflictStart.toLocaleString('en-US', formatOptions);
                const conflictEnd = availabilityResult.conflictEnd.toLocaleString('en-US', formatOptions);
...
async function proceedWithBooking(event) {
    ...
    try {
        ...
        const availabilityResult = await checkAvailability();
        if (!availabilityResult.available) {
            alert(availabilityResult.error || "Car is not available for the selected time");
            if (proceedButton) {
            ...
            setTimeout(() => {
                ...
                setTimeout(() => {
                    ...
                    setTimeout(() => {
                        ...
                        // Last resort
                        alert("Please click OK to continue to the booking confirmation page.");
                    }, 300);
                }, 300);
            }, 300);
        } catch (navError) {
            console.error("Navigation error:", navError);
            alert("Unable to automatically redirect. Please manually navigate to the booking confirmation page.");
        }
    } catch (error) {
