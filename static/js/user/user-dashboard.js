...
function initializeTimeSelectors(currentHour, currentMinute) {
    console.log("Initializing time selectors with:", { currentHour, currentMinute });
...
function updateMinutesOptions(minutesSelect, selectedHour) {
    ...
    const firstValidOption = [...minutesSelect.options].find(option => !option.disabled);
    if (firstValidOption) {
...
async function loadActiveBookings(userId) {
    try {
        ...
        // Show loading state
        bookingsContainer.innerHTML = '<div class="loading-state"><div class="spinner"></div><p>Loading your bookings...</p></div>';
        ...
        const bookingPromises = carsSnapshot.docs.map(async carDoc => {
            ...
            return Promise.all(carBookingsSnapshot.docs.map(async bookingDoc => {
            ...
            function createBookingElement(booking) {
                ...
                const formattedStartDate = startDate.toLocaleDateString('en-US', { weekday: 'short', month: 'short', day: 'numeric' });
                ...
                const carImage = carInfo.image || '../static/assets/images/car-placeholder.jpg';
                ...
                bookingCard.innerHTML = `
                    <div class="booking-status ${statusClass}">
                        ...
                        ${timeRemainingDisplay ? `<div class="time-remaining">${timeRemainingDisplay}</div>` : ''}
                    </div>
                    <div class="booking-content">
                        ...
                        <div class="booking-details">
                            <h3 class="booking-title">${carInfo.make || ''} ${carInfo.modelName || 'Car'}</h3>
                    ...
                    for (const carDoc of carsSnapshot.docs) {
                        ...
                        if (carData.status && carData.status.toLowerCase() === 'available') {
                            ...
                            if (carData.current_location && 
                                ...
                                withLocationCount++;
                                console.log(`Car ${carDoc.id} has valid location:`, carData.current_location);
            ...
            function createCarElement(car) {
                ...
                const priceDisplay = car.price_per_hour ? `$${car.price_per_hour.toFixed(2)}/hour` : '';
                
                // Create HTML content
                carEl.innerHTML = `
                    <div class="car-image">
                        <img src="${car.image}" alt="${car.make} ${car.modelName}" onerror="this.src='../static/assets/images/car-placeholder.jpg';">
                    </div>
                    <div class="car-info">
                        <div class="car-header">
                            <h3>${car.make} ${car.modelName}</h3>
                            <div class="car-type-badge">${baseCarType || 'Standard'}</div>
                        </div>
                        <div class="car-meta">
                            ${distanceDisplay}
                        ...
                        if (car.current_location &&
                            ...
                            try {
                                const marker = new google.maps.Marker({
                                    position: position,
                                    map: map,
                                    title: `${car.make || ''} ${car.modelName || car.car_type || 'Car'}`,
                                    icon: {
                                ...
                                const infoWindow = new google.maps.InfoWindow({
                                    content: `
                                        <div class="map-info-window">
                                            <h4>${car.make || ''} ${car.modelName || car.car_type || 'Car'}</h4>
                                ...
                                console.error(`Error adding marker for car ${car.id || index}:`, error);
                            }
                        } else {
                            console.warn(`Car ${car.id || index} has invalid location:`, car.current_location);
                        }
                    });
