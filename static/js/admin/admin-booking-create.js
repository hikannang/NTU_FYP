// admin-booking-create.js - Part 1
...
let hourlyRate = 20; // Default hourly rate in dollars
...
async function loadUsersData() {
  ...
  try {
    ...
    usersSnapshot.forEach(doc => {
      ...
      options += `<option value="${userData.id}">${displayName} (${email})</option>`;
    });
...
function setupEventListeners() {
  ...
  const dateTimeInputs = [
    startDateInput, startTimeHourInput, startTimeMinuteInput,
...
// Toggle maintenance booking mode
function toggleMaintenanceMode() {
  ...
  if (isMaintenanceBooking) {
    ...
    // Show user selection for regular bookings
...
function handleCarSelection() {
  ...
  if (carDetailsElement && selectedCar) {
    carDetailsElement.innerHTML = `
      <div class="car-detail"><strong>Model:</strong> ${selectedCar.car_type || 'N/A'}</div>
      <div class="car-detail"><strong>License:</strong> ${selectedCar.license_plate || 'N/A'}</div>
      <div class="car-detail"><strong>Color:</strong> ${selectedCar.car_color || 'N/A'}</div>
    `;
  ...
  // Check car availability for the selected time period
...
function handleUserSelection() {
  ...
  if (userDetailsElement && selectedUser) {
    userDetailsElement.innerHTML = `
      ...
      <div class="user-detail"><strong>Phone:</strong> ${selectedUser.phone || 'N/A'}</div>
    `;
...
async function checkCarAvailability() {
  ...
  try {
    ...
    const bookingsRef = collection(db, "timesheets", selectedCar.id, "bookings");
...
function validateForm() {
  ...
  if (!isMaintenanceBooking && !selectedUser) {
