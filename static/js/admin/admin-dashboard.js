      ...
      if (bookingData.car_id) {
        try {
          ...
          if (carDoc.exists()) {
            ...
            if (carData.car_type) {
              carDetails = `${carData.car_type} (${
                carData.license_plate || "No Plate"
              })`;
    ...
    carsSnapshot.forEach((doc) => {
      ...
      if (car.service_due) {
        ...
        if (daysDiff < 0) {
          ...
          // Changed from 7 to 14 days (two weeks notice)
      ...
      if (car.insurance_expiry) {
        ...
        if (daysDiff < 0) {
          ...
          // Changed from 14 to 30 days (one month notice)
          // Expiring soon
          alerts.push({
            id: `${doc.id}-insurance`,
            title: `${car.car_type || "Car"} (${
  ...
  dashboardData.recentBookings.forEach((booking) => {
    ...
    tableContent += `
      ...
      booking.status
        ...
        <td>
          <button class="action-btn" onclick="viewBookingDetails('${
            booking.id
          }', '${booking.car_id}')">
            <i class="bi bi-eye"></i>
          </button>
