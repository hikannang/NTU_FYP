...
function handleOrientation(event) {
    ...
    // Note: direction is now calculated in calculateARDirection/calculateGPSDirection
    // and applied in updateUI
}

// Calculate bearing between current position and destination
function calculateBearing() {
    if (!current.latitude || !current.longitude || !destination.latitude || !destination.longitude) {
        return 0;
    }
...
function calculateGPSDirection() {
    ...
    if (smoothedHeading !== null) {
        ...
        // Log occasionally
        if (Math.random() < 0.01) {
            console.log("GPS-based direction:", directionAngle.toFixed(1) + "°", 
...
// Update UI to show arrow direction
function updateUI() {
...
function createDestinationMarker(lat, lng) {
    ...
    try {
        ...
        entity.setAttribute('animation', 'property: position; to: 0 1 0; dur: 2000; dir: alternate; loop: true; easing: easeInOutQuad');
        
        // Add a loaded event to know when the AR is ready
        entity.addEventListener('loaded', function() {
            console.log("AR marker loaded successfully - arrow will now track AR position");
            arEntityLoaded = true;
        });
...
function updateDistanceDisplay() {
    ...
    if (distance < MODAL_TRIGGER_DISTANCE && !isViewed) {
        console.log(`Within ${MODAL_TRIGGER_DISTANCE} meters of destination, showing modal`);
...
function addDebugInfo() {
    ...
    setInterval(() => {
        debugElement.innerHTML = `
            <div><b>Current:</b> ${current.latitude?.toFixed(6) || 'N/A'}, ${current.longitude?.toFixed(6) || 'N/A'}</div>
            <div><b>Target:</b> ${target.latitude?.toFixed(6) || 'N/A'}, ${target.longitude?.toFixed(6) || 'N/A'}</div>
            ...
            <div><b>AR Entity:</b> ${arEntityLoaded ? 'Loaded' : 'Not loaded'}</div>
